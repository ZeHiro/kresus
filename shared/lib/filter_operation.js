...
export default function filter(operation, searchObject) {
    ...
    the operation to.
        fields of searchObject:
            ...
            dateLow: minimum value of the date of the operation
    ...
    */
    ...
    if (maybeHas(searchObject, 'category') && searchObject.category  &&
    operation.categoryId !== searchObject.category) {
        return false;
    }

    if (maybeHas(searchObject, 'type') && searchObject.type  &&
    operation.operationTypeID !== searchObject.type) {
        return false;
    }

    if (maybeHas(searchObject, 'amountHigh') && searchObject.amountHigh &&
    operation.amount > searchObject.amountHigh) {
        return false;
    }

    if (maybeHas(searchObject, 'amountLow') && searchObject.amountLow &&
    operation.amount < searchObject.amountLow) {
        return false;
    }

    if (maybeHas(searchObject, 'dateLow') && searchObject.dateLow &&
    new Date(operation.date) < new Date(searchObject.dateLow)) {
        return false;
    }

    if (maybeHas(searchObject, 'dateHigh') && searchObject.dateHigh &&
    new Date(operation.date) > new Date(searchObject.dateHigh)) {
        return false;
    }
